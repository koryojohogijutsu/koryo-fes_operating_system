...
export default function AdminMapPage() {
  ...
  // Next.js のリンク遷移はbeforeunloadが効かないのでLinkをラップして警告
  const handleNavAway = useCallback((e: React.MouseEvent) => {
    if (isDirty && !confirm("保存されていない変更があります。このページを離れますか？")) {
      e.preventDefault();
    }
  }, [isDirty]);
  ...
  return (
    <main style={{ padding: "20px", maxWidth: "800px", margin: "0 auto" }}>
      {/* 離脱リンク（Next.js Linkは onClick で警告） */}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
        ...
        {isDirty && (
          <span style={{ fontSize: "12px", color: "#ff9800", fontWeight: "bold", backgroundColor: "#fff8e1", padding: "4px 10px", borderRadius: "20px", border: "1px solid #ffe082" }}>
            ● 未保存の変更あり
          </span>
        )}
      </div>
