...
export default function EventEnterPage() {
  ...
  const scanningRef = useRef(false); // スキャン処理中のガード用
  ...
  const [scanning, setScanning] = useState(false); // カメラ起動中フラグ
  const [isInitializing, setIsInitializing] = useState(false); // 起動処理中フラグ
  ...
  const startScanner = async () => {
    ...
    // 1. まずUI上でスキャナー領域を表示させる
    setScanning(true);

    // 2. DOMの描画を待つために少し遅延を入れる（確実にID="event-reader"を見つけるため）
  ...
  return (
    <main style={{ padding: "24px 20px", maxWidth: "480px", margin: "0 auto", backgroundColor: "#fff", minHeight: "100vh", color: "#333" }}>
      <a href="/" style={{ fontSize: "13px", color: "#888", textDecoration: "none", display: "block", marginBottom: "20px" }}>
        ← ホームに戻る
      </a>

      <h1 style={{ fontSize: "20px", marginBottom: "6px", color: "#000" }}>🎤 イベント</h1>
      ...
      {/* カメラ映像表示エリア：常にDOMに配置し、scanningの状態に合わせて表示/非表示を切り替え */}
      <div
        style={{
      ...
      <section style={{ marginTop: "12px" }}>
        <h2 style={{ fontSize: "15px", fontWeight: "bold", marginBottom: "10px", borderBottom: "2px solid #e10102", paddingBottom: "6px", color: "#000" }}>
          入場履歴
        </h2>
        {loadingHistory ? (
          ...
          <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
            {eventVisits.map((v, i) => (
              <li key={i} style={{ display: "flex", justifyContent: "space-between", padding: "12px 0", borderBottom: "1px solid #f0f0f0", fontSize: "14px" }}>
                <strong style={{ color: "#333" }}>{v.event_label}</strong>
