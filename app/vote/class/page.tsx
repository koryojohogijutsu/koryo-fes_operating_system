...
export default function VoteClassPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const entered = [...new Set((visits ?? []).map((v) => v.class_code))] as string[];
  ...
  const getClassesForCategory = (grade: number | null) => {
    return allClasses.filter((c) => {
      const isEntered = enteredClasses.includes(c.code);
      if (!isEntered) return false;
      if (grade === null) return true; // 装飾賞は全学年
