...
export default function VoteEventCategoryPage() {
  ...
  return (
    <main style={{ padding: "24px 20px", maxWidth: "480px", margin: "0 auto" }}>
      <a href="/event-enter" style={{ fontSize: "13px", color: "var(--muted)", textDecoration: "none", display: "block", marginBottom: "20px" }}>
        ← 戻る
      </a>
      <h1 style={{ fontSize: "20px", marginBottom: "6px" }}>{EVENT_LABELS[category]}</h1>
      <p style={{ color: "var(--muted)", fontSize: "13px", marginBottom: "28px" }}>投票する出場者を選んでください</p>
      {entries.length === 0 ? (
        <p style={{ color: "var(--muted)", fontSize: "14px" }}>出場者が登録されていません</p>
      ) : (
          ...
          {entries.map((e) => {
            ...
            return (
              <div key={e.id} onClick={() => setSelected(isSelected ? "" : e.id)}
                style={{ padding: "14px 16px", borderRadius: "10px", border: isSelected ? "2px solid #e10102" : "1px solid #ddd", backgroundColor: isSelected ? "rgba(225,1,2,0.08)" : "var(--card-bg)", cursor: "pointer" }}>
