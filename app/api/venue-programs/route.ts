...
export async function GET(req: NextRequest) {
  ...
  let query = supabase.from("venue_programs").select("id, venue_key, name, datetime, comment").order("datetime", { nullsFirst: false });
...
export async function POST(req: Request) {
  const { venueKey, name, datetime, comment } = await req.json();
  ...
  const { error } = await supabase.from("venue_programs").insert({
    venue_key: venueKey, name, datetime: datetime ?? "", comment: comment ?? "", order_num: Date.now(),
  });
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
